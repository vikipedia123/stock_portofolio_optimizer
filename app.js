const els = {
  ...
  forecastDownside: document.querySelector("#forecastDownside"),
};
...
function buildForecastAnalytics() {
  model.forecastMeanReturns = forecastReturnVector(model.returns);
  model.forecastCovariance = ewmaCovarianceMatrix(model.returns, model.forecastMeanReturns);
  model.forecastCurrent = forecastStats(model.weights);
  model.forecastSimulations = simulateForecastPortfolios(4500);
  model.forecastMaxSharpe = model.forecastSimulations.reduce((best, item) => (item.sharpe > best.sharpe ? item : best), model.forecastSimulations[0]);
  model.forecastMinVol = model.forecastSimulations.reduce((best, item) => (item.risk < best.risk ? item : best), model.forecastSimulations[0]);
  model.forecastPath = simulateForwardPaths(model.forecastMaxSharpe.weights, 1000);
}

function forecastReturnVector(rows) {
  const columns = transpose(rows);
  return columns.map((values) => {
    const ewma = exponentiallyWeightedMean(values, 0.94);
    const recent = mean(values.slice(-20));
    const medium = mean(values.slice(-60));
    const blended = 0.55 * ewma + 0.3 * recent + 0.15 * medium;
    return clamp(blended, -0.015, 0.015);
  });
}

function exponentiallyWeightedMean(values, decay) {
  let weighted = 0;
  let totalWeight = 0;
...
function ewmaCovarianceMatrix(rows, forecastMeans) {
  const decay = 0.94;
  const count = rows.length;
  return forecastMeans.map((leftMean, i) => forecastMeans.map((rightMean, j) => {
    let weighted = 0;
    let totalWeight = 0;
    rows.forEach((row, rowIndex) => {
      const age = count - rowIndex - 1;
      const weight = Math.pow(decay, age);
      weighted += weight * (row[i] - leftMean) * (row[j] - rightMean);
      totalWeight += weight;
    });
    return weighted / totalWeight;
  }));
...
function forecastStats(weights) {
  const days = forecastHorizon();
  const riskFree = Number(els.riskFree.value || 0) / 100;
  const dailyReturn = weights.reduce((sum, weight, index) => sum + weight * model.forecastMeanReturns[index], 0);
  const horizonReturn = Math.pow(1 + dailyReturn, days) - 1;
  const dailyVariance = weights.reduce((outer, weightI, i) => (
    outer + weights.reduce((inner, weightJ, j) => inner + weightI * weightJ * model.forecastCovariance[i][j], 0)
  ), 0);
  const horizonRisk = Math.sqrt(Math.max(0, dailyVariance) * days);
  const horizonRiskFree = riskFree * (days / 252);
  const sharpe = horizonRisk === 0 ? 0 : (horizonReturn - horizonRiskFree) / horizonRisk;
  return { weights: [...weights], return: horizonReturn, risk: horizonRisk, sharpe };
}
...
function simulateForecastPortfolios(count) {
  const simulations = [forecastStats(model.weights), forecastStats(equalWeights(model.symbols.length))];
  for (let i = 0; i < count; i += 1) {
    simulations.push(forecastStats(randomWeights(model.symbols.length)));
  }
  return simulations;
}

function simulateForwardPaths(weights, pathCount) {
  const days = forecastHorizon();
  const paths = [];
  const rowWeights = recencyWeights(model.returns.length, 0.992);

  for (let p = 0; p < pathCount; p += 1) {
    const path = [100];
    for (let day = 0; day < days; day += 1) {
      const sampled = model.returns[weightedIndex(rowWeights)];
      const bootstrapReturn = weights.reduce((sum, weight, index) => sum + weight * sampled[index], 0);
      const drift = weights.reduce((sum, weight, index) => sum + weight * model.forecastMeanReturns[index], 0);
      const blendedReturn = 0.35 * drift + 0.65 * bootstrapReturn;
      path.push(path.at(-1) * (1 + blendedReturn));
    }
    paths.push(path);
  }
...
function renderMetrics() {
  ...
  els.forecastReturn.textContent = pct(model.forecastMaxSharpe.return);
  els.forecastDownside.textContent = pct(model.forecastPath.at(-1).p10 / 100 - 1);
...
function updateWeightsFromInputs() {
  ...
  model.forecastSimulations[0] = model.forecastCurrent;
  renderAll();
}
...
function renderRebalanceTable() {
  ...
  const rows = model.symbols.map((symbol, index) => {
    ...
    const targetValue = model.forecastMaxSharpe.weights[index] * capital;
    ...
    return `
      <tr>
        ...
        <td>${pct(model.forecastMaxSharpe.weights[index])}</td>
...
function renderForecastTable() {
  const rows = [
    ["Current forecast", model.forecastCurrent],
    ["Forecast Max Sharpe", model.forecastMaxSharpe],
    ["Forecast Min Vol", model.forecastMinVol],
  ].map(([label, item]) => `
    <tr>
      <td><span class="tag">${label}</span></td>
      <td>${pct(item.return)}</td>
  ...
  els.forecastTable.innerHTML = `
    <table>
      <thead><tr><th>30-day portfolio</th><th>Forecast return</th><th>Forecast risk</th><th>Sharpe</th><th>Largest weights</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  `;
}
...
function drawForecast() {
  ...
  clearCanvas(ctx, canvas);
  const pad = { l: 58, r: 22, t: 24, b: 48 };
  const points = model.forecastPath;
  const allValues = points.flatMap((point) => [point.p10, point.p50, point.p90]);
  const xScale = scaler(0, forecastHorizon(), pad.l, canvas.width - pad.r);
  const yScale = scaler(Math.min(...allValues), Math.max(...allValues), canvas.height - pad.b, pad.t);
  drawAxes(ctx, canvas, pad, "Trading days", "Simulated value");

  ctx.fillStyle = "rgba(36, 88, 211, 0.13)";
  ctx.beginPath();
  points.forEach((point, index) => {
    const x = xScale(point.day);
    const y = yScale(point.p90);
    if (index === 0) ctx.moveTo(x, y);
    else ctx.lineTo(x, y);
  });
  [...points].reverse().forEach((point) => {
    ctx.lineTo(xScale(point.day), yScale(point.p10));
  });
  ctx.closePath();
  ctx.fill();

  drawLine(ctx, points.map((point) => [xScale(point.day), yScale(point.p50)]), "#2458d3", 3);
  drawLine(ctx, points.map((point) => [xScale(point.day), yScale(point.p10)]), "#c94747", 1.5);
  drawLine(ctx, points.map((point) => [xScale(point.day), yScale(point.p90)]), "#139b65", 1.5);
  ctx.fillStyle = "#182033";
  ctx.fillText(`Forecast Max Sharpe: ${weightSummary(model.forecastMaxSharpe.weights)}`, pad.l + 10, pad.t + 18);
}
...
function forecastHorizon() {
  return Math.max(5, Math.min(90, Math.round(Number(els.forecastDays.value || 30))));
}

function recencyWeights(count, decay) {
  const weights = [];
  let total = 0;
...
function weightedIndex(cumulativeWeights) {
  const draw = Math.random();
  const index = cumulativeWeights.findIndex((value) => value >= draw);
  return index === -1 ? cumulativeWeights.length - 1 : index;
}
